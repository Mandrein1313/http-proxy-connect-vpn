...
public class SettingsActivity extends AppCompatActivity {
    ...
    private EditText sshHostInput, sshPortInput, sshUserInput, sshPassInput;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sshUserInput = findViewById(R.id.sshUserInput);
        sshPassInput = findViewById(R.id.sshPassInput);
        ...
        // ดึงค่า SSH เดิมมาแสดง
        sshHostInput.setText(prefs.getString("ssh_host", ""));
        sshPortInput.setText(String.valueOf(prefs.getInt("ssh_port", 22)));
        sshUserInput.setText(prefs.getString("ssh_user", ""));
        sshPassInput.setText(prefs.getString("ssh_pass", ""));
        ...
        btnSave.setOnClickListener(v -> {
            int port = 22;
            try {
                port = Integer.parseInt(sshPortInput.getText().toString().trim());
            } catch (Exception ignored) {}

            prefs.edit()
                    .putString("ssh_host", sshHostInput.getText().toString().trim())
                    .putInt("ssh_port", port)
                    .putString("ssh_user", sshUserInput.getText().toString().trim())
                    .putString("ssh_pass", sshPassInput.getText().toString().trim())
